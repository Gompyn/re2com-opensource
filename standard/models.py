...
def multi_encoder(encoder_inputs, encoders, encoder_input_length, other_inputs=None, **kwargs):
    ...
    with tf.variable_scope('decoder_nl'):
        fused = dense(tf.concat([code, ast], axis=1), 2*encoder.cell_size, use_bias=False, activation=None, name='fuse')
    encoder_state = fused * (1-sim_score) + exemplar * sim_score
...
def encoder_decoder(encoders, decoders, encoder_inputs, targets, feed_previous, align_encoder_id=0,
    ...
    attention_states, encoder_state, encoder_input_length, sim_score = multi_encoder(
